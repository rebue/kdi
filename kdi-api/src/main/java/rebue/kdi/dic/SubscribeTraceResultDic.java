 ...
 *
 */
public enum SubscribeTraceResultDic implements EnumBase {
	/**
	 * 1: 订阅成功
	 */
	SUCCESS(1),
	 ...
	 */
	@JsonCreator
	public static SubscribeTraceResultDic getItem(int code) {
		EnumBase result = valueMap.get(code);
		if (result == null) {
			throw new IllegalArgumentException("输入的code" + code + "不在枚举的取值范围内");
		}
		return (SubscribeTraceResultDic) result;
	}
	 ...
	 * 构造器，传入code
	 */
	SubscribeTraceResultDic(int code) {
		this.code = code;
	}
