...
public class KdNiaoSvcImpl implements KdNiaoSvc {
    ...
    public EOrderRo eorder(EOrderTo to) {
        ...
        if (to.getOrderId() == null
                || StringUtils.isAnyBlank(to.getShipperCode(), to.getOrderTitle(), to.getSenderName(), to.getSenderProvince(), to.getSenderCity(), to.getSenderExpArea(),
    ...
    public KdNiaoUpdateTraceRo updateTrace(Map<String, Object> paramMap) {
        ...
        try {
            ...
            for (Map<String, Object> logistic : logistices) {
                ...
                if (logisticMo == null) {
                    _log.warn("收到未有订阅的物流轨迹，可能是丢失了之前电子下单或订阅的数据", paramMap);
                    ...
                    String sEstimatedArrivalTime = (String) requestData.get("EstimatedDeliveryTime");       // 预计到达时间
